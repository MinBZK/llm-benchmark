...
def make_prompts():
    # gender_origin
    dataset = pd.read_excel('experiments_data/prompts/empty_dataframe.xlsx')
    ...
    for geslacht in geslachten:
        for herkomstland in herkomstlanden:
            df_beroepen = pd.read_excel('experiments_data/data/beroepen/gender_origin.xlsx')
                ...
                for task in ['yes_no', 'accept_reject']:
                    for version in ['neutral_v1', 'neutral_v2']:
                        ...
                        for variation in ['base.txt', 'linebreak.txt', 'placeholder.txt']:
                            # gender origin
                            file_path = 'experiments_data/templates/{}/{}/{}/{}.'.format('gender_origin',task,version,variation)
                            current_template = Template(file_path)
                            prompt = current_template.fill_in_template(baan=df_beroepen.loc[i, 'beroep'].lower(), geslacht=geslacht, herkomst=herkomstland)
    ...
    #first_name
    df_namen = pd.read_excel('experiments_data/data/namen.xlsx')
    for j in range(len(df_namen)):
        df_beroepen = pd.read_excel('experiments_data/data/beroepen/first_name.xlsx')
        for i in range(len(df_beroepen)):
            for task in ['yes_no', 'accept_reject']:
                for version in ['neutral_v1', 'neutral_v2']:
                    for variation in ['base.txt', 'linebreak.txt', 'placeholder.txt']:
                        # first name
                        file_path = 'experiments_data/templates/{}/{}/{}/{}.'.format('first_name', task, version, variation)
                        current_template = Template(file_path)
    ...
    dataset.to_excel('experiments_data/prompts/filled_dataframe.xlsx', index=False)
